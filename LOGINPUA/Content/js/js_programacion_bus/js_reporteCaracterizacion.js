...
function getResumenProgramacion() {
    ...
    $.ajax({
        ...
        data: { idServicio: $('#selectRutaConsulta').val() },
        dataType: 'json',
        success: function (result) {
    ...
    var strHTML = '';
            ...
            $('#tbReporteCaracterizacion tbody').empty();
            if (result.length <= 0) {
                ...
                $.each(result, function (i) {
                    if (this.CANTIDAD_VIAJE > 0) {
                        ...
                        strHTML += '<tr data-maestro="' + this.ID_MAESTRO_SALIDA_PROG + '">' +
                                        '<td>' + (i + 1) + '</td>' +
            ...
            $('#tbReporteCaracterizacion tbody').append(strHTML);
...
function verPOG(element) {
    ...
    var idruta = $('#selectRutaConsulta').text()
    var idMaestroSalidaProg_ = element.parent().parent().attr('data-maestro')
...
function exportarTabla() {
    ...
    console.log(tableDataExportar, 'tableDataExportar')
...
function AgregarCamposPOG() {
    ...
    var idruta = $("#selectRutaConsulta").val();
