...
{
    public class ProduccionDemandaAD
    {
        OracleConnection conn;

        public ProduccionDemandaAD(ref Object _bdConn)
        {
        ...
        public DataSet Listar_Informes(int mes, int año)
        {
            ...
            using (var bdConn = new OracleConnection(Configuracion.GetConectionSting("sConexionSISCC")))
            {
                using (OracleCommand command = new OracleCommand())
                {
                    ...
                    command.CommandText = "PKG_PRODUCCION_DEMANDA.SP_LISTAR_INFORMES";
        ...
        public RPTA_GENERAL nombre_excel(int id)
        {
            ...
            using (var bdCmd = new OracleCommand("PKG_PRODUCCION_DEMANDA.NOM_EXCEL", conn))
            {
        ...
        public RPTA_GENERAL eliminar_Archivo(int id)
        {
            ...
            try
            {
                using (var bdCmd = new OracleCommand("PKG_PRODUCCION_DEMANDA.ELIMINAR_ARCHIVO", conn))
                {
                    bdCmd.CommandType = CommandType.StoredProcedure;
                    bdCmd.Parameters.AddRange(bdParameters);
                    bdCmd.Connection.Open();
        ...
        public RPTA_GENERAL ValidarFecha_Programados(string fecha)
        {
            ...
            using (var bdCmd = new OracleCommand("PKG_PRODUCCION_DEMANDA.VALIDARFECHA", conn))
            {
...

        {
            ...
            try
            {
                using (var bdCmd = new OracleCommand("PKG_PRODUCCION_DEMANDA.SP_INSERTAR_INFORMES", conn))
                {
                    bdCmd.CommandType = CommandType.StoredProcedure;
                    bdCmd.Parameters.AddRange(bdParameters);
                    bdCmd.Connection.Open();
